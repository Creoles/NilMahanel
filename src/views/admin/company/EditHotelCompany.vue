    ...
    methods: {
      submit() {
        ...
        if (this.$route.params.id) {
          this.$http.put('hotel/company/' + this.$route.params.id, this.createModel).then(res => {
          ...
          this.$http.post('/hotel/company/create', this.createModel).then(res => {
            if (res.data.code === 200) {
              this.submitting = false;
              this.$message({
                type: 'success',
              ...
              this.$router.push({name: "HOTEL COMPANY"});
            } else {
              this.$message({
                type: 'error',
                message: res.data.message
      ...
      loadCompany(){
        if (this.$route.params.id) {
          this.loading = true;
          this.$http.get('/hotel/company/' + this.$route.params.id).then(res => {
            if (res.data.code === 200) {
